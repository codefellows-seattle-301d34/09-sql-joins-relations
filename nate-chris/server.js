...
// DONE: These are routes for requesting HTML resources.
app.get('/new-article', (request, response) => {
  response.sendFile('new.html', {root: './public'});
});

// DONE: These are routes for making API calls to enact CRUD operations on our database.
...
app.put('/articles/:id', function(request, response) {
  let SQL = 'UPDATE authors SET author = $2, "authorUrl" = $3 WHERE author_id = $1;'
  let values = [
  ...
  client.query( SQL, values )
    .then(() => {
      let SQL = 'UPDATE articles SET author_id=$2, title=$3, category=$4, "publishedOn"=$5, body=$6 WHERE article_id=$1';
      let values = [
        request.params.id,
        request.body.author_id,
        request.body.title,
